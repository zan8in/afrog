...
func IsRestrictIP(ipStr string) bool {
	...
	// IPv4 Check
	for _, cidr := range DefaultIPv4DenylistRanges {
	...
	// IPv6 Check
	for _, cidr := range DefaultIPv6DenylistRanges {
		_, ipNet, _ := net.ParseCIDR(cidr)
		if ip.To16() != nil && ipNet.Contains(ip) {
			return true
		}
	}

	// Special Address Check 169.254.169.254
