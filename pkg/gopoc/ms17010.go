...
func smbMS17010(args *GoPocArgs) (Result, error) {
	// init pocinfo & result
	poc := poc.Poc{
		Id: smbMS17010Name,
		Info: poc.Info{
	...
	if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
		...
		result.IsVul = true
		url := proto.UrlType{Host: args.Host, Port: "445"}
		result.SetAllPocResult(true, &url, []byte(""), []byte(fmt.Sprintf("%s is likely VULNERABLE to MS17-010!\n", ip)))
		return result, nil
	}
