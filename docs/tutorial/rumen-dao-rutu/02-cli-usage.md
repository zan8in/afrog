...
# 只盯一个服务（示例：SSH 弱口令）
afrog -t 192.168.1.0/24 -ps -p 22 -s ssh-weak-login

# 先把弱口令 PoC 列出来看看（用于了解支持范围）
afrog -pl -s weak-login,default
```
...
### 3. 列出/查看 PoC：`-pl` / `-pd`
```bash
# 按关键词列出 PoC（注意：-s 只匹配 PoC 的 id/name）
...
## 🧾 Afrog 所有命令表（CLI 参数一览）

下面按分组列出 Afrog 的所有 CLI 参数（以当前版本源码为准）。

### Target

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `-t`, `--target` |  | 目标（URL/Host），逗号分隔 |
| `-T`, `--target-file` |  | 目标文件（每行一个） |
| `-cs`, `--cyberspace` |  | 空间测绘类型（示例：`-cs zoomeye`） |
| `-q`, `--query` |  | 空间测绘语句（示例：`-q app:'tomcat'`） |
| `-qc`, `--query-count` | `100` | 空间测绘拉取数量（示例：`-qc 1000`） |
| `--resume` |  | 断点续扫（`resume.afg`） |
| `--oob` |  | 指定 OOB 适配器（示例：`ceyeio`/`dnslogcn`/`alphalog`） |

### PoCs

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `-P`, `--poc-file` |  | 指定 PoC 文件或目录 |
| `-ap`, `--append-poc` |  | 追加 PoC 文件或目录（逗号分隔） |
| `-pd`, `--poc-detail` |  | 查看某个 PoC 详情（按 PoC ID） |
| `-pl`, `--poc-list` | `false` | 列出 PoC 列表 |
| `-ep`, `--exclude-pocs` |  | 排除 PoC（逗号分隔） |
| `-epf`, `--exclude-pocs-file` |  | 排除 PoC 文件（每行一个） |

### Output

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `-o`, `--output` |  | 输出 HTML 报告（包含漏洞结果） |
| `-j`, `--json` |  | 输出 JSON（不包含请求/响应内容） |
| `-ja`, `--json-all` |  | 输出 JSON（包含完整漏洞结果） |
| `-doh`, `--disable-output-html` | `false` | 禁用自动生成 HTML（优先级高于 `-o`） |
...
| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `-rl`, `--rate-limit` | `150` | 全局每秒最大请求数 |
| `-rlt`, `--req-limit-per-target` | `0` | 每目标（host:port）每秒请求数，`0` 关闭 |
| `--auto-req-limit` | `false` | 自动设置 per-target 限速 |
| `--polite` | `false` | 以 polite 策略设置 per-target 限速 |
| `--balanced` | `false` | 以 balanced 策略设置 per-target 限速 |
| `--aggressive` | `false` | 以 aggressive 策略设置 per-target 限速 |
| `-c`, `--concurrency` | `25` | 并发 PoC 数 |
| `--smart` | `false` | 根据资产数量变化智能调整并发 |
| `-orl`, `--oob-rate-limit` | `25` | OOB PoC 每秒最大请求数 |
| `-oc`, `--oob-concurrency` | `25` | OOB PoC 最大并发数 |
...
| `--retries` | `1` | 请求失败重试次数 |
| `--timeout` | `50` | 超时时间（秒） |
| `--mt` | `false` | 启用 monitor-target |
| `--mhe` | `3` | 单主机最大错误数（超过后跳过） |
| `--mrbs` | `2` | HTTP 响应体大小上限 |
| `--brute-max-requests` | `5000` | 单规则最大爆破请求数，`0` 禁用 |
| `--silent` | `false` | 静默模式（仅结果） |
| `-nf` | `false` | 禁用指纹阶段（跳过 tags 含 `fingerprint` 的 PoC） |
| `-w` | `false` | 启用 webprobe 阶段（Web 存活探测） |
| `--fingerprint-filter-mode` | `strict` | 指纹过滤模式：`strict` / `opportunistic` |
| `--pedm` | `false` | PoC 执行耗时监控 |
| `--pedm-log-limit` | `50` | `-pedm` 时记录前 N 个 started 任务（`0` 关闭） |
| `--pedm-slow-sec` | `3` | `-pedm` 时记录慢于 N 秒的任务 |
| `--pedm-slow-log-limit` | `50` | `-pedm` 时最多记录 N 条慢任务 |
| `--pedm-summary-top` | `20` | `-pedm` 时输出最慢 Top N 汇总（`0` 关闭） |
| `--pedm-summary-by` | `max` | `-pedm` 汇总排序：`max` / `avg` |
| `--vsb` | `false` | 命中漏洞后立即停止扫描 |
| `-H`, `--header` |  | 全局请求头/Cookie（可重复） |
| `--http-default-accept` | `true` | PoC 未设置 Accept 时自动加 `Accept: */*` |
| `--sort` |  | 扫描排序（示例：`-sort a-z`） |

### Update

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `-un`, `--update` | `false` | 更新 afrog 引擎到最新版本 |
| `-duc`, `--disable-update-check` | `false` | 禁用自动更新检查 |

### Proxy

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `--proxy` |  | HTTP/SOCKS5 代理（逗号分隔或文件输入） |

### Debug

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `--debug` | `false` | 打印全部请求与响应 |
| `--test` | `false` | 测试模式（需要 gating disabled） |
| `--live-stats` | `false` | 单行实时状态渲染 |
| `-v`, `--version` | `false` | 显示版本信息 |
| `--validate` |  | 校验 PoC YAML 语法（文件/目录） |

### Server

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `--web` | `false` | 启动 Web Server |

### PortScan（端口预扫）

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `-ps`, `--portscan` | `false` | 启用资产端口预扫 |
| `-p`, `--ports` | `top` | 端口定义（如 `80,443,1000-2000` 或 `top/full/all`） |
| `-Pn`, `--ps-skip-discovery` | `false` | 跳过主机发现，直接端口扫描 |
| `-prate`, `--ps-rate` | `0` | 端口预扫速率限制 |
| `-ptimeout`, `--ps-timeout-ms` | `0` | 端口预扫超时（毫秒） |
| `-ptries`, `--ps-retries` | `0` | 端口预扫重试次数 |
| `--ps-s4-chunk` | `1000` | `ports=full` 时 S4 分块大小 |

### Webhook

| 参数 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `--dingtalk` | `false` | 启用钉钉 Webhook |
| `--wecom` | `false` | 启用企业微信 Webhook |
...
| `--curated-endpoint` |  | curated 服务 endpoint |
| `--curated-timeout` | `0` | curated 挂载超时（秒） |
| `--curated-force-update` | `false` | 立即强制检查 curated 更新 |
