...
### 4) 补救措施：`-mt` 参数有用吗？
很多同学会问：**“加了 `-mt` (Monitor Targets) 能不能把死目标救活？”**

**结论：能救“假死”，救不了“真死”。**

- **`-mt` 的作用**：它会在扫描前（或扫描中）对目标进行**持续的、带重试的**探活检测。
- **适用场景**：
  - 网络不稳定（丢包率高）。
  - 目标服务器响应慢。
  - 目标有偶尔的连接拒绝。
- **它能做什么**：如果是网络抖动导致第一次探活失败，`-mt` 会多试几次（默认 3 次），只要有一次成功，就会把目标标记为“存活”，交给引擎去扫。
- **它还能加速**：对于真正的死目标，`-mt` 会在后台快速识别并将其“拉黑”。主扫描线程在遇到被拉黑的目标时会**直接跳过**，不再浪费时间等待超时。这能有效防止大量死目标拖慢整体扫描进度。
- **它不能做什么**：如果目标服务器真的关机了，或者端口真的没开，`-mt` 试 100 次也是失败。

> **🛑 关于“拉黑”的终局性**
> 
> 用户常问：**“如果一个目标被拉黑了，还有机会‘复活’吗？”**
> **答案：本次扫描中没有机会。**
> 
> 一旦某个目标的失败次数超过阈值（默认为 3 次，可通过 `-mhe` 参数修改），Afrog 就会给它打上“永久拉黑”的标签（内部状态标记为 `Blacklisted`）。
> - **主线程**：遇到它直接跳过。
> - **监控线程**：停止对它的探测。
> - **结果**：直到扫描结束，这个目标都不会再被触碰。这是为了确保扫描效率，防止在一个死目标上无限死磕。

**建议**：在公网扫描或网络环境较差时，**强烈建议开启 `-mt`**，它可以显著减少因为网络波动导致的漏扫。但它不是“硬扫”的开关，**“硬扫”还得靠手动指定完整 URL**。
